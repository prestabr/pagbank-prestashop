...
class PagBank extends PaymentModule
{
	...
	public function getContent()
	{
		...
		$token_ps = $this->getTokenHash();
		$sort_number = $this->sortRefNumber();
		...
		$this->context->smarty->assign(array(
			...
			'new_user_state_d14' => urlencode(basename(_PS_ADMIN_DIR_) . '/registerUser/D14/' . $token_ps . '/' . $sort_number),
			'new_user_state_d30' => urlencode(basename(_PS_ADMIN_DIR_) . '/registerUser/D30/' . $token_ps . '/' . $sort_number),
			'new_user_state_tax' => urlencode(basename(_PS_ADMIN_DIR_) . '/registerUser/TAX/' . $token_ps . '/' . $sort_number),
	...
	*/
	...
	public function getAppAuthorization($code, $app)
	{
		...
		if (isset($resp_object) && !isset($resp_object->error_messages)) {
			...
			$this->saveLog('error', $app, false, $json_authorization, $resp, $post_url);
			return false;
		}
	}
	...
	public function refreshToken($refreshToken, $app)
	{
		...
		if (isset($resp_object) && !isset($resp_object->error_messages)) {
			...
			$this->saveLog('error', $app, false, $json_refresh, $resp, $post_url);
			return false;
		}
	}
