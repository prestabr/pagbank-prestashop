...
<div id="pagbank_config" class="panel">
	<div class="row moduleconfig-header">
		<div class="col-xs-12 col-sm-3 text-center" align="center">
			<a class="pagbank" href="https://pagbank.com.br/" target="_blank"
	...
	<div class="row">
		...
		<div id="collapseCron" class="collapse in" data-parent="#accordionCron">
			<div class="info-cron">
				...
				<p>
					<b>{l s='URL:' mod='pagbank'}</b> {$callback_url}?action=cancelNotPaidPix&token={$token_ps}
				</p>
				<p class="comment">
				...
				<p>
					<b>{l s='URL:' mod='pagbank'}</b> {$callback_url}?action=cancelNotPaidBankslip&token={$token_ps}
				</p>
				<p class="comment">
				...
				<p>
					<b>{l s='URL:' mod='pagbank'}</b> {$callback_url}?action=cancelNotPaidWallet&token={$token_ps}
				</p>
				<p class="comment">
					...
					<div class="card {if (isset($tokenD14) && $tokenD14 != false)}already-registered{/if}">
						<div class="card-body text-center">
							...
							<p class="text-muted">
								...
								<i class="icon-check"></i> {$splAppInfoD14[1]}
							</p>
							<div class="icon-box icon-box--primary">
							...
							<p>
								...
								{else}
									<a href="{$signin_connect_url}?response_type=code&client_id={$client_id_d14}&redirect_uri={$callback_url}&scope={$scope}&state={$new_user_state_d14}&code_challenge={$code_challenge_d14}&code_challenge_method=S256"
					...
					<div class="card {if (isset($tokenD30) && $tokenD30 != false)}already-registered{/if}">
						<div class="card-body text-center">
							...
							<p class="text-muted">
								...
								<i class="icon-check"></i> {$splAppInfoD30[1]}
							</p>
							<div class="icon-box icon-box--primary">
								{l s='Boleto Bancário - 2 dias' mod='pagbank'}
							</div>
							...
							<p>
								...
								{else}
									<a href="{$signin_connect_url}?response_type=code&client_id={$client_id_d30}&redirect_uri={$callback_url}&scope={$scope}&state={$new_user_state_d30}&code_challenge={$code_challenge_d30}&code_challenge_method=S256"
					...
					<div class="card {if (isset($tokenTax) && $tokenTax != false)}already-registered{/if}">
						<div class="card-body text-center">
							...
							<p>
								...
								{else}
									<a href="{$signin_connect_url}?response_type=code&client_id={$client_id_tax}&redirect_uri={$callback_url}&scope={$scope}&state={$new_user_state_tax}&code_challenge={$code_challenge_tax}&code_challenge_method=S256"
